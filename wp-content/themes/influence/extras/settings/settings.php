...
function siteorigin_settings_admin_menu() {
	$theme = wp_get_theme();
	$page = add_theme_page(
		sprintf(__( '%s Theme Settings', 'influence' ), $theme->get('Name') ),
		sprintf(__( 'Theme Settings', 'influence' ), $theme->get('Name') ),
		'edit_theme_options',
		'theme_settings_page',
...
function siteorigin_settings_render() {
	if( version_compare( get_bloginfo('version'), '3.4', '<' ) ) {
		?><div class="wrap"><div id="setting-error-settings_updated" class="updated settings-error"><p><strong><?php _e('Please update to the latest version of WordPress to use theme settings.', 'influence') ?></strong></p></div></div><?php
		return;
	}
...
function siteorigin_settings_enqueue_scripts( $prefix ) {
	...
	wp_localize_script( 'siteorigin-settings', 'siteoriginSettings', array(
		'premium' => array(
			...
			'name' => apply_filters('siteorigin_premium_theme_name', ucfirst( get_option( 'template' ) ) . ' ' . __( 'Premium', 'influence' ) ),
		),
		'tour' => array(
			'buttonText' => __('Theme Tour', 'influence'),
	...
	switch ( $args['type'] ) {
		case 'checkbox' :
			...
			<div class="checkbox-wrapper">
				...
				<label for="<?php echo esc_attr( $field_id ) ?>"><?php echo esc_attr( !empty( $args['label'] ) ? $args['label'] : __( 'Enabled', 'influence' ) ) ?></label>
			</div>
			<?php
			break;
		case 'text' :
		...
		case 'media':
			if(version_compare(get_bloginfo('version'), '3.5', '<')){
				printf(__('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'influence'), admin_url('update-core.php'));
				break;
			}
			...
			$choose_title = empty($args['choose']) ? __('Choose Media', 'influence') : $args['choose'];
			$update_button = empty($args['update']) ? __('Set Media', 'influence') : $args['update'];
			...
			?>
				<div class="media-field-wrapper">
					...
					<a href="#" class="media-remove-button"><?php _e('Remove', 'influence') ?></a>
				</div>
		...
		case 'gallery' :
			...
			<input
				...
				type="text"
				value="<?php echo esc_attr( $current ) ?>" />
			<a href="#" class="so-settings-gallery-edit"><?php _e('Select Images', 'influence') ?></a>
			<?php
			break;
		...
		case 'pages' :
			...
			?>
			<select id="<?php echo esc_attr( $field_id ) ?>" name="<?php echo esc_attr( $field_name ) ?>">
				<option value="0"><?php _e('None', 'influence') ?></option>
		...
		default :
			_e( 'Unknown Field Type', 'influence' );
			break;
	}
...
function siteorigin_settings_change_message() {
	if ( get_theme_mod( 'siteorigin_settings_changed' ) ) {
		...
		<div id="setting-updated" class="updated">
			<p>
				<strong><?php _e( 'Settings saved.', 'influence' ) ?></strong>
			</p>
		</div>
		<?php
...
function siteorigin_settings_theme_help(){
	...
	$text = sprintf(
		__( "Read %s's <a href='%s' target='_blank'>theme documentation</a> for help with these settings.", 'influence' ),
		ucfirst($theme_name),
	...
	$screen->add_help_tab( array(
		...
		'title' => __( 'Settings Help', 'influence' ),
		'content' => '<p>' . $text . '</p>',
	) );
}
...
function siteorigin_settings_media_view_strings($strings, $post){
	...
	$strings['insertIntoPost'] = __('Set Media File', 'influence');
...
function siteorigin_settings_add_slider_options($options){
	...
	if( class_exists('MetaSliderPlugin') ){
		...
		foreach($sliders as $slider) {
			$options['[metaslider id="'.$slider->ID.'"]'] = __('Meta Slider: ', 'influence').$slider->post_title;
		}
	}
	...
	if( function_exists('rev_slider_shortcode') ) {
		...
		foreach($sliders as $slider) {
			$options['[rev_slider '.$slider->alias.']'] = __('Revolution Slider: ', 'influence').$slider->title;
		}
	}
	...
	if( function_exists('layerslider') ) {
		...
		foreach($sliders as $slider) {
			$options['[layerslider id="'.$slider->id.'"]'] = __('LayerSlider: ', 'influence').$slider->name;
		}
	}
