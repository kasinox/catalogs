...
function accesspress_parallax_paging_nav() {
	...
	<nav class="navigation paging-navigation" role="navigation">
		...
		<div class="nav-links">
			...
			<div class="nav-previous"><?php next_posts_link( __( '<i class="fa fa-angle-left"></i> Older Posts', 'accesspress_parallax' ) ); ?></div>
			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<div class="nav-next"><?php previous_posts_link( __( 'Newer Posts <i class="fa fa-angle-right"></i>', 'accesspress_parallax' ) ); ?></div>
			<?php endif; ?>
...
function accesspress_parallax_post_nav() {
	...
	<nav class="navigation post-navigation" role="navigation">
		<div class="nav-links">
			<?php
				previous_post_link( '<div class="nav-previous">%link</div>', '<i class="fa fa-angle-left"></i>%title' , TRUE );
				next_post_link(     '<div class="nav-next">%link</div>',     '%title<i class="fa fa-angle-right"></i>' , TRUE );
			?>
		</div><!-- .nav-links -->
	</nav><!-- .navigation -->
	<?php
...
function accesspress_parallax_posted_on() {
	...
	if($post_author ==  '1') :
	echo '<span class="posted-by"> ' . $byline . '</span>';
	endif;
...
if ( ! function_exists( 'accesspress_parallax_single_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function accesspress_parallax_single_posted_on() {
	$time_string = '<time class="entry-date published" datetime="%1$s"><span class="posted-day">%2$s</span> <span class="posted-month">%3$s</span></time>';
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string .= '<time class="updated" datetime="%4$s">%5$s</time>';
	}
	...
	$single_post_date = of_get_option('single_post_date');
	$single_post_author = of_get_option('single_post_author');

	if($single_post_author ==  '1') :
	echo '<span class="byline"> ' . $byline . '&nbsp;&nbsp;</span>';
	endif;

	if($single_post_date ==  '1' ) :
	echo '<span class="posted-on">' . $posted_on . '</span>';
	endif;
